...
thr_data_full <- read.csv(thr_data_file)
del_data_full <- read.csv(del_data_file)
inst_thr_data_full <- read.csv(inst_thr_data_file)

# specify to this flow
thr_data <- subset(thr_data_full, flow == flow_num)
del_data <- subset(del_data_full, flow == flow_num)
inst_thr_data <- subset(inst_thr_data_full, flow == flow_num)


# add a line/point plot
thr_plot <- ggplot(thr_data, aes(x=time,y=throughput)) + geom_line()
...
######## inst throughput ######
inst_thr_plot <- ggplot(inst_thr_data, aes(x=time,y=throughput)) + geom_line()
...
##### del plot ######
# add a line/point plot
del_plot <- ggplot(del_data, aes(x=time,y=rtt)) + geom_line()
